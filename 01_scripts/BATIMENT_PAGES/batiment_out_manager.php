...
<html lang="fr">
    ...
    <body>
        ...
        <header class="sticky-top">
            <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
                <div class="container-fluid">
                    ...
                    <ul class="navbar-nav">
                        <li class="nav-item"><a class="nav-link active" href="batiment_add_checklist.php"><i class="fa-solid fa-person-through-window fa-xl"></i> Ajouter Entrée</a></li>
                        <li class="nav-item"><a class="nav-link active" href="batiment_homepage.php"><i class="fa-solid fa-person-walking-arrow-right fa-xl"></i> Suivi Entrées</a></li>
                        <li class="nav-item"><a class="nav-link active" href="batiment_closedentry.php"><i class="fa-solid fa-list-check fa-xl"></i> Entrées Terminées</a></li>
                        <li class="nav-item"><a class="nav-link active" href="batiment_out_manager.php"><i class="fa-solid fa-person-walking-arrow-right fa-flip-horizontal fa-xl"></i> Suivi Sorties</a></li>
        ...
        <h4 style="margin-top: 40px; margin-bottom: 30px;"><u>LISTE DES NOVATIONS</u></h4>
            ...
            if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_equipement_novation']) && isset($_POST['id_sortie_novation'])) {
                $id_sortie = intval($_POST['id_sortie_novation']);
                // Mettre à jour la colonne out_cle à 'Terminer'
                $sql_update = "UPDATE sorties_table SET out_cle = 'Terminer' WHERE id_sortie = $id_sortie";
                mysqli_query($database_connect, $sql_update);
                // Vérifier si out_cle est 'Terminer'
                $sql_check = "SELECT out_cle FROM sorties_table WHERE id_sortie = $id_sortie";
                $result_check = mysqli_query($database_connect, $sql_check);
                if ($result_check) {
                    $equip = mysqli_fetch_assoc($result_check);
                    if ($equip['out_cle'] === 'Terminer') {
                        $sql_state = "UPDATE sorties_table SET state_batiment = 1 WHERE id_sortie = $id_sortie";
            ...
            $nov_query_count = "SELECT COUNT(*) as total FROM sorties_table WHERE type_sortie='NOVATION' AND state_batiment = 0";
            ...
            $list_out = mysqli_query($database_connect, "SELECT * FROM sorties_table WHERE type_sortie='NOVATION' AND state_batiment = 0 ORDER BY date_sortie ASC LIMIT $nov_limit OFFSET $nov_offset");
            ?>
            <table class="table table-hover table-bordered" style="width: 100%; margin-bottom: 40px;font-size: 14px;">
                ...
                while ($row=$list_out->fetch_assoc()) {
                    ...
                    echo '<tr style="height: 40%; vertical-align: middle;">';
                        ...
                        if ($row['out_cle'] == "A faire" || $row['out_cle'] == "A verifier") {
                            ...
                            echo '<img class="icon_1" src="../../01_assets/fa-cle.png" alt="Clé" title="Clé">';
                            echo '</button>';
                            echo '</form>';
                        } else {
                            echo '<img class="icon_1" src="../../01_assets/fa-cle - Copie.png" alt="Clé" title="Clé" style="margin:8px;">';
                        }
                        echo "</td>";
            ...
            if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_equipement_sortie']) && isset($_POST['id_sortie_sortie'])) {
            ...
            $sort_query_count = "SELECT COUNT(*) as total FROM sorties_table WHERE type_sortie='SORTIE' AND state_batiment = 0";
            ...
            $list_out = mysqli_query($database_connect, "SELECT * FROM sorties_table WHERE type_sortie='SORTIE' AND state_batiment = 0 ORDER BY date_sortie ASC LIMIT $sort_limit OFFSET $sort_offset");
            ?>
            <table class="table table-hover table-bordered" style="width: 100%; margin-bottom: 40px;font-size: 14px;">
                <tr class="table-dark" style="font-size: 14px;">
                    <th style="width: 8%">Société</th>
